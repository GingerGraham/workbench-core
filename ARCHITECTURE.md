...
| D38 | Bash-native scheduled-sync timer install, Ansible dependency removed for this path (host debugging session — scheduled sync never ran on a real host with no ansible-playbook installed) | A confirmed regression, not hypothetical: the systemd `--user` timer/launchd agent that fires `wb sync run-if-due` lived entirely inside the Ansible `module_sync` role, reached only when `ansible-playbook` is on `PATH`. On a host without it, `wb install`/`wb apply` logged a routine-looking "ansible-playbook not found — skipping the Ansible convergence step" and scheduled sync silently never ran, indefinitely — `wb sync enable <module>` gave a false-positive "already true — no-op" because it only ever checks the `SYNC_ENABLED` config flag, never whether the OS timer actually exists. Inconsistent with §9.1's existing zero-Ansible-dependency sync-engine hot path, and with this project's general direction of not depending on Ansible where it isn't earning its keep — the unit/plist content here is static with one substitution (the path to `wb`), nothing Ansible's templating engine was actually needed for. Fixed by moving timer install into `lib/sync/scheduler.sh` (`workbench_scheduler_install`), called unconditionally from `wb install`/`wb apply` on every platform (Linux/WSL2 via `systemctl --user`, macOS via `launchctl`), independent of whether Ansible is present at all. The Ansible `module_sync` role's now-superseded "Scheduled sync" tasks and `templates/{systemd,launchd}/` were removed; its unrelated module-registration task, and the entirely separate `sync-external` role's own hourly timer for `external_synced_repos`, are untouched. The fixed-5-minute-poll-plus-userspace-self-throttle design itself (`contracts/tracking-spec.md` §Cadence) is unchanged — only where the unit files get written from moved. Ansible remains in place for whatever else it still does (host_vars templating, module registration parity via `--skip-ansible`-gated convergence) — this decision scopes only to the scheduled-sync timer, not a full removal of Ansible from the project. |
