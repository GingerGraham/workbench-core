...
Automated semver/tagging/release pipeline, D27. `.github/workflows/
pr-check.yml` and `release.yml`, backed by `.github/scripts/release/`, turn
Conventional-Commit-formatted merges to `main` into version bumps, tags,
and GitHub Releases without a manual cut. See `docs/release-process.md`
for the full mechanics — the Conventional Commit scoping rules, the
`core`-scope convention, the CHANGELOG discipline this builds on, and what
to do if a release fails its own safety net.
