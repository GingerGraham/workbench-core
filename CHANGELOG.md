...
- **Module names and `REPO_URL` values are now validated.** A module name
  ...
  restricts it to lowercase letters, digits and `-`, starting with a
  letter or digit, at most 64 characters. `REPO_URL` is passed to `git
  ls-remote`/`git remote add` — `workbench_valid_repo_url` now allowlists
  the URL shapes those calls actually understand, so a value starting
  with `-` (which `git` would otherwise parse as an option) is rejected.
  Both are checked by `wb add`, catalog bundle resolution (an invalid
...
- **`wb completion bash|zsh`**: tab-completion for wb's top-level
  subcommands, generated by introspecting bin/wb's own dispatch case (no
  hand-maintained command list to drift). Wired up automatically via
  shell/completions/wb.sh, a version-stamped cache keyed on
  workbench-core's own release version — same pattern as the
  gh.sh/uv.sh-style tool-completion wrappers other modules ship. Phase 1:
  top-level commands only, no argument-level completion yet.
  `tests/check-wb-completion.sh`. See `ARCHITECTURE.md` §12, D44.
  `wb completion` is also added to the shell wrapper's purely-informational
  command list (`lib/loader.sh`), so running it doesn't trigger a needless
  shell reload (D39).
